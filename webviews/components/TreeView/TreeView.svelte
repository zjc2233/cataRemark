<script>
   ...
   * @typedef {{ resourcePath: TreeNodeId; text: string; icon?: typeof import("svelte").SvelteComponent; disabled?: boolean; expanded?: boolean; }} TreeNode
  ...
  /**
   * Set the current active node resourcePath
  ...
  export function collapseNodes(filterNode = (node) => true) {
    expandedIds = nodes
      .filter((node) => expandedIds.includes(node.resourcePath) && !filterNode(node))
      .map((node) => node.resourcePath);
  }
  ...
  setContext("TreeView", {
    ...
    expandedNodeIds,
    clickNode: (node) => {
      activeId = node.resourcePath;
      selectedIds = [node.resourcePath];
    ...
    expandNode: (node, expanded) => {
      if (expanded) {
        expandedIds = [...expandedIds, node.resourcePath];
      } else {
        expandedIds = expandedIds.filter((_id) => _id !== node.resourcePath);
      }
    },
...
{#if !hideLabel}
  ...
  <!-- <label resourcePath="{labelId}" class:bx--label="{true}">
